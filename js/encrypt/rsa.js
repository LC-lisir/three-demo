...
RSA.biDivideModulo = function(x, y) {
	...
	if (r.digits[0] == 0 && RSA.biHighIndex(r) == 0) r.isNeg = false;
...
RSA.biPow = function(x, y) {
	...
	var a = x;
	while (true) {
		if ((y & 1) != 0) result = RSA.biMultiply(result, a);
		y >>= 1;
		if (y == 0) break;
...
RSA.biPowMod = function(x, y, m) {
	...
	var k = y;
	while (true) {
		if ((k.digits[0] & 1) != 0) result = RSA.biMultiplyMod(result, a, m);
		k = RSA.biShiftRight(k, 1);
		if (k.digits[0] == 0 && RSA.biHighIndex(k) == 0) break;
